...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        s,l = sentence[:max_length], labels[:max_length]
        m = [True] * len(s)        
        diff = max_length - len(s)
        s += [zero_vector] * diff
        l += [zero_label] * diff
        m += [False] * diff
        ret.append((s,l,m))
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                h = tf.zeros(name='h', shape = [self.config.batch_size, self.config.hidden_size])
                if time_step > 0:
        ...
        preds = tf.pack(values=preds)
        preds = tf.transpose(preds, perm=[1,0,2])
        ### END YOUR CODE

        assert preds.get_shape().as_list() == [self.config.batch_size, self.max_length, self.config.n_classes], "predictions are not of the right shape. Expected {}, got {}".format([self.config.batch_size, self.max_length, self.config.n_classes], preds.get_shape().as_list())
        return preds
