...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("transformation"):
            bias = tf.Variable(tf.zeros_initializer([self.config.n_classes]))
            W = tf.Variable(tf.zeros_initializer([self.config.n_features,self.config.n_classes]))
            z = tf.matmul(self.input_placeholder, W) + bias 
        # for 'z', see note on assignment and look at dimensions of input; 
        # the desc. above has 'W' and 'x' switched
        pred = softmax(z)
        ### END YOUR CODE
        return pred
